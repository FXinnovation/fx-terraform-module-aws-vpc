...
output "public_subnets_count" {
  ...
  value       = "${var.create_vpc && length(var.public_subnets) > 0 && (!var.one_nat_gateway_per_az || length(var.public_subnets) >= length(var.azs)) ? length(var.public_subnets) : 0}"
}

output "database_subnets_count" {
  description = "Count of database subnets"
  value       = "${var.create_vpc && length(var.database_subnets) > 0 ? length(var.database_subnets) : 0}"
}

output "intra_subnets_count" {
  description = "Count of intra subnets"
  value       = "${var.create_vpc && length(var.intra_subnets) > 0 ? length(var.intra_subnets) : 0}"
}

output "public_route_table_count" {
  description = "Count of public route tables"
  value       = "${var.create_vpc && length(var.public_subnets) > 0 ? 1 : 0}"
}

output "private_route_table_count" {
  description = "Count of private route tables"
  value       = "${var.create_vpc && local.max_subnet_length > 0 ? local.nat_gateway_count : 0}"
}
...
output "database_route_table_count" {
  ...
  value       = "${var.create_vpc && var.create_database_subnet_route_table && length(var.database_subnets) > 0 ? 1 : 0}"
}

output "intra_route_table_count" {
  description = "Count of public route tables"
  value       = "${var.create_vpc && length(var.intra_subnets) > 0 ? 1 : 0}"
}
